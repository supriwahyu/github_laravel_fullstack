      ...
      <div class="w-full px-6 py-4 bg-white rounded shadow-md ring-1 ring-gray-900/10">
        <form method="POST" action="#">
          ...
          <div>
            ...
            <h3 class="text-2xl font-semibold">{{ $books->name }}</h3>
            <div class="flex items-center mb-4 space-x-2">
              <span class="text-xs text-gray-500"> {{ $books->detail }}</span><span class="text-xs text-gray-500">
                {{ $books->price }}</span>
            </div>
            <p class="text-base text-gray-700">{{$books->stock}}</p>
          </div>
        </form>
      </div>
    </div>
